  ...
  if (variant === 'cinema-12') {
    return (
      <article
        ...
        className={`group relative flex flex-col pt-10 sm:pt-14 pb-16 sm:pb-20 border-b border-[#E2DFD2] ${
          computedColSpan || 'lg:col-span-12'
        } ${computedOffset || ''}`}
      >
        {/* 1. Project Number & Top Annotation */}
        <div className="flex items-baseline justify-between gap-4 mb-4 pb-2 border-b border-[#E2DFD2]/60 text-xs font-mono">
          <div className="flex items-baseline gap-3">
            <span className="font-serif text-4xl sm:text-5xl lg:text-6xl font-light text-[#171717]/25 tracking-tight select-none">
              {project.number}
            </span>
            <span className="text-[11px] uppercase font-mono tracking-wider text-[#6F87AA] font-semibold">
              {project.type}
            </span>
            <span className="text-[#9E9A90]">·</span>
            <span className="text-[12px] text-[#67645C] font-mono">{project.year}</span>
          </div>

          {handwrittenNote && (
            <span className="font-['Caveat',cursive] text-lg sm:text-xl text-[#6F87AA] -rotate-1 select-none hidden sm:inline">
              ~ {handwrittenNote}
            </span>
          )}
        </div>

        {/* 2. Dominant Large Visual Artifact */}
        <Link
          to={`/projects/${project.slug}`}
          className="block mb-7 focus:outline-none"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-12 items-baseline">
          <div className="lg:col-span-7 space-y-3">
            <Link to={`/projects/${project.slug}`} className="group/title block">
              <h2 className="font-serif text-3xl sm:text-4xl lg:text-[48px] font-light leading-[1.05] tracking-tight text-[#171717] lowercase transition-all duration-300">
            ...
            <p className="text-[16px] sm:text-[17px] text-[#67645C] font-sans leading-relaxed max-w-2xl">
              {summary}
            </p>
          </div>
          ...
          <div className="lg:col-span-5 space-y-4 lg:text-right flex flex-col lg:items-end justify-between h-full">
            {/* Tools Stack */}
            <div className="text-xs font-mono text-[#67645C] space-y-1">
              <span className="text-[10px] uppercase tracking-wider text-[#9E9A90] block">built with:</span>
              <span className="text-[#171717] font-medium">{project.tools.slice(0, 4).join(' · ')}</span>
            </div>

            {/* Small Action */}
            <div className="flex flex-wrap items-center gap-4 pt-2 text-[12px] font-sans">
              {project.demo && (
                <a
                  href={project.demo}
                  target="_blank"
              ...
              <MagneticLink strength={3}>
                <Link
                  to={`/projects/${project.slug}`}
                  className="group/link inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-[#171717] text-[#F5F4ED] hover:bg-[#6F87AA] text-[12px] uppercase tracking-[0.14em] transition-colors"
                >
  ...
  return (
    ...
    >
      <div>
        ...
        <div className="flex items-baseline justify-between mb-4 pb-2 border-b border-[#E2DFD2]/60 text-[12px] font-sans text-[#67645C]">
        ...
        <Link
          to={`/projects/${project.slug}`}
          className="block mb-5 focus:outline-none"
          aria-label={`View project details for ${title}`}
        >
          <EditorialBrowserFrame
            project={project}
      ...
      <div className="space-y-3 pt-1">
        <Link to={`/projects/${project.slug}`} className="group/title block">
          <h2 className="font-serif text-2xl sm:text-3xl lg:text-[34px] font-light leading-[1.08] tracking-tight text-[#171717] lowercase transition-all duration-300">
            <span className="relative inline-block transition-transform duration-300 group-hover/title:translate-x-1 group-hover/title:italic">
              {title}
        ...
        {/* Tools Stack */}
        <div className="text-xs font-mono text-[#9E9A90]">
          <span className="text-[#171717]">{project.tools.slice(0, 3).join(' · ')}</span>
        </div>

        {/* Small Action */}
        <div className="pt-2 flex flex-wrap items-center justify-between gap-3 text-[12px] font-sans border-t border-[#E2DFD2]/40">
